...
const Form = () => {
  ...
  async function handleBuyNFT() {
    if (!tokenId || tokenId==="" || receiver==="" || !receiver) return;

    const provider = new ethers.providers.Web3Provider(window.ethereum);
    const signer = provider.getSigner();
    const contract = new ethers.Contract(CONTRACT_ADDRESS, Marketplace.abi, signer);

    try {
      const price = await contract.getListingPrice();
      const transaction = await contract.sellNFT(tokenId, receiver, price);
